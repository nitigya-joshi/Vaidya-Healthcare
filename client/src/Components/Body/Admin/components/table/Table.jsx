...
const Table = ({ listtype }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (listtype === "approve") {
      const doctorres = await fetch('http://localhost:3000/api/doctors/unapproved', {
        credentials: 'include'
      })
  ...
  const deleteUsers = async (mongo_ids) => {
    try {
      if (listtype === "users") {
        const res = await fetch('http://localhost:3000/api/users/deleteUser', {
          method: 'POST',
          credentials: 'include',
      ...
      if (listtype === "doctors") {
        const res = await fetch('http://localhost:3000/api/doctors/deleteDoctor', {
          method: 'POST',
          credentials: 'include',
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const approveUsers = async (details) => {
    try {
      if (listtype === "approve") {
        const res = await fetch('http://localhost:3000/api/doctors/approve', {
          method: 'POST',
          credentials: 'include',
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
